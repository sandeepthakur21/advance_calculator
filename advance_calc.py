...
root = Tk()
root.title('Advance calc')


#addig the input field
display = Entry(root)
display.grid(row=1,columnspan=6,sticky=N+S+E+W)

#code to add buttons to the calc
...
Button(root,text=")",command = lambda :get_variable(")")).grid(row=4,column=5,sticky=N+S+E+W)
...
# i keeps the track of current poition on the input text field
i=0
